...
<div class="mb-8">
  ...
  <p class="text-gray-600 mt-1">Perbarui informasi produk camping</p>
</div>
...
<!-- Form -->
<form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
  @csrf
  @method('PUT')
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
    ...
    <div class="md:col-span-2">
      ...
      <input type="text" name="name" value="{{ $product->name }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition duration-200" required>
    </div>

    <!-- Harga -->
    <div>
      <label class="block text-sm font-semibold text-gray-900 mb-2">Harga (Rp / Hari)</label>
      <input type="number" name="price_per_day" value="{{ $product->price_per_day }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition duration-200" required>
    </div>
  ...
  <div class="mb-6">
    ...
    <textarea name="description" rows="5" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition duration-200" required>{{ $product->description }}</textarea>
  </div>
  ...
  <div class="mb-8">
    ...
    <div class="mb-4">
      ...
      <img src="{{ asset('storage/'.$product->image) }}" alt="{{ $product->name }}" class="h-32 w-32 rounded-lg object-cover border border-gray-200 shadow-sm">
    </div>
    @endif
    ...
    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-cyan-500 hover:bg-cyan-50 transition duration-200" onclick="document.getElementById('image-input').click()">
      ...
      <p class="text-gray-600 font-medium">Klik untuk ubah gambar</p>
      <p class="text-gray-400 text-sm">Biarkan kosong jika tidak ingin mengubah</p>
    </div>
