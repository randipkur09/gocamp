      ...
      <tbody class="divide-y divide-gray-200">
        ...
        <tr class="hover:bg-gray-50 transition duration-200">
          ...
          <td class="px-6 py-4 text-sm flex justify-center gap-2">
            <a href="{{ route('admin.products.edit', $p->id) }}" class="px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition duration-200 font-medium text-xs">
              Edit
            </a>
            <form action="{{ route('admin.products.destroy', $p->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus produk ini?')">
              @csrf
              @method('DELETE')
              <button type="submit" class="px-3 py-1.5 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition duration-200 font-medium text-xs">
                Hapus
              </button>
            </form>
          </td>
